...
def create_master_df(transactions_df, customers_df, products_df) -> object:
    try:
        ...
        master_df['date_of_purchase'] = pd.to_datetime(master_df['date_of_purchase'])
...
def main():
    try:
        ...
        # master_df.to_csv(os.path.join(params.get('output_location'), 'output_df.csv'))

        master_df.drop(['product_description', 'date_of_purchase'], axis=1, inplace=True)

        # dfw = master_df.resample('W')
        var = master_df.groupby(['customer_id', 'loyalty_score', 'product_id', 'product_category'])['price'].describe()[['count']]
        #var = pd.DataFrame(var)
        var.columns = var.columns.str.replace('count', 'purchase_count')
        var = var.reset_index()

        j = (var.groupby(['customer_id', 'loyalty_score'])
             .apply(lambda x: x[['product_id', 'product_category', 'purchase_count']].to_dict('records'))
             .reset_index()
